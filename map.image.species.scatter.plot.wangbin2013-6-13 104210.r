...
#````````````````````````````````````````````````````` 
 ...
 tiff(paste(nam00[i],'.tiff'), width = 2100, height = 1700,res=600,compression = "lzw")
     par(mex=0.45,mar=c(3,3,3,0.5))
     contour(xx, yy,zz, levels = seq(round(z.min), z.max, by = 20),
             add = F, col = "peru",lwd=1,labcex=0.1, cex.axis=1, yaxt="n")
...
tiff('MapPlotImageNongGang.tiff',
     width = 3800, height = 2750,res=600,compression = "lzw")
...
axis(1, cex.axis=1, at = seq(3, 500, length.out = 11), labels= seq(0, 500, by = 50))
axis(2, cex.axis=1, at = seq(3, 300, length.out = 7), labels = seq(0, 300, by = 50))
box()
op1 <- par( fig=c(.9, 1 ,0 ,1), new=T, mex=0.3, mar=c(7,6,1,1))
...
#
tiff('BrachMapPlot-0-5.tiff',
     width = 3800, height = 2800,res=600,compression = "lzw")
...
#
tiff('BrachMapPlot-5-20-inf.tiff',
     width = 3800, height = 2800,res=600,compression = "lzw")
      op0 <- par(mex=0.5,mar=c(3,3,1,1))
